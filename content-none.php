...
<article <?php post_class('post'); ?> itemscope itemtype="http://schema.org/Article">
	...
	<div class="entry-content" itemprop="articleBody">
		...
		<?php if ( is_home() && current_user_can( 'publish_posts' ) ) : ?>
			...
			<p><?php printf( __( 'Ready to publish your first post? <a href="%1$s">Get started here</a>.', 'toutatis' ), admin_url( 'post-new.php' ) ); ?></p>
		
		<?php elseif ( is_search() ) : ?>
			
			<p><?php _e('Sorry but no post match what you are looking for. Try some new keywords below :','toutatis'); ?></p>
			<?php get_search_form(); ?>
		
		<?php else: ?>
		
			<p><?php printf( __('Sorry but no post match what you are looking for. You should <a href="%1$s">go back to the homepage</a> and start again.','toutatis'), home_url()); ?></p>
